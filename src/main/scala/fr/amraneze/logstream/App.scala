...
object App {
	...
	def main(args: Array[String]): Unit = {
		val df: DataFrame = sparkSession.read.text(appConfig.data.input)
		splitCorrectlyAndWrite(df, appConfig.data.validOutput, appConfig.data.rejectedOutput)
	}

	def splitCorrectlyAndWrite(inputDf: DataFrame,
							   outputValidPath: String,
							   outputRejectedPath: String): Unit = {

		import sparkSession.implicits._

		val logs = inputDf.map { row => parseLog(row.getAs[String](0)) }

		val validDf: DataFrame = logs.filter(_.isRight).map {
			_.fold(_ => "", log => log.ip)
		}.toDF()
